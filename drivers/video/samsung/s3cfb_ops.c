...
{
	struct s3cfb_window *win = fb->par;
	struct fb_fix_screeninfo __attribute__((unused)) prev_fix = fb->fix;
	struct fb_var_screeninfo __attribute__((unused)) prev_var = fb->var;
...
int s3cfb_ioctl(struct fb_info *fb, unsigned int cmd, unsigned long arg)
{
	...
	struct s3cfb_window *win = fb->par;
	struct s3cfb_global *fbdev = get_fimd_global(win->id);
	struct s3cfb_lcd __attribute__((unused)) *lcd = fbdev->lcd;
	void *argp = (void *)arg;
	int ret = 0;
#if defined(CONFIG_CPU_EXYNOS4210)
	unsigned int addr = 0;
